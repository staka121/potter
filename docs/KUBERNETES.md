...
- 🏠 **Local Development**: Docker Compose + gateway-service
- ☁️ **Production**: Kubernetes + Ingress
- 📝 **Single Source**: One `.tsubo.yaml` file for both environments
...
- ✅ Fast: No cluster setup required
- ✅ Unified: Single entry point (localhost:8080)
...
- ✅ Resilient: Self-healing and rolling updates
- ✅ Production-grade: Battle-tested Ingress Controllers
- ✅ Secure: TLS termination, network policies
...
- Service name → API path inference
...
```
my-app/
├── app.tsubo.yaml          # Application definition
├── user-service.object.yaml # Contract definitions
├── todo-service.object.yaml
└── k8s/                    # Generated (gitignored)
    ├── namespace.yaml
    ├── deployment-*.yaml
    ├── service-*.yaml
    └── ingress.yaml
...
> **Potter bridges WHERE (Docker Compose ⇄ Kubernetes)**
